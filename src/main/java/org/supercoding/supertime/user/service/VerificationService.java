...
public class VerificationService {
    ...
    public void certificationRequest(User user, MultipartFile image) {
        ...
        validateCertificationState(userEntity.getVerified());
        authStateEntity.setAuthImageId(addImage(image));

        setCertificationState(userEntity, authStateEntity);
    }

    private void validateCertificationState(Verified userVerified) {
        switch (userVerified){
            case DENIED:
    ...
    public void certificationResubmission(User user, MultipartFile image) {
        ...
        validateCertificationStateForResubmission(userEntity.getVerified());
        authStateEntity.setAuthImageId(addImage(image));

        setCertificationState(userEntity, authStateEntity);
    }

    private void validateCertificationStateForResubmission(Verified userVerified) {
        switch (userVerified){
            case NEEDED:
    ...
    private void setCertificationState(UserEntity userEntity, AuthStateEntity authStateEntity) {
        userEntity.setVerified(Verified.PENDING);
