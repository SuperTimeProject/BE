...
public class AuthService {
    ...
    public void signup(SignupRequestDto signupInfo) {
        ...
        long[] userBoard = createUserBoardList(userSemester.getSemesterName());
    ...
    private long[] createUserBoardList(int semesterName) {
        long[] userBoard = new long[3];
        String[] boardList = {"전체 게시판", "커뮤니티 게시판", "기수 게시판 (" + semesterName + ")"};

        for (int i = 0; i < boardList.length; i++) {
            userBoard[i] = userValidation.findBoard(boardList[i]).getBoardCid();
        }
